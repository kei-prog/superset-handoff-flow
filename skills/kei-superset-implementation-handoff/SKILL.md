...
description: 現在のタスクで明確になった問題、期待結果、判断、制約をkei-handoffで実装担当エージェント向けの指示へ変換し、使用するCodexモデルと推論レベルとともに、分かりやすい日本語で全文を事前提示する。ユーザーの明示承認後だけSuperset workspace、agent、またはCodex CLI terminalを起動する。ユーザーが「Supersetで実装を渡して」「Superset経由でCodexに依頼」「kei-superset-implementation-handoff」と依頼した時に使う。実装を止める未確定事項が残る場合は起動せず、必要な確認を1つ返す。
---

# Kei Superset Implementation Handoff

現在のタスクの引継ぎ情報を実装指示へ変換し、モデル、推論レベル、指示全文を分かりやすい日本語でユーザーに確認してからSupersetで起動し、Codexが実際に開始した証拠まで確認する。

標準のユーザー操作は、問題と要件を会話で明確化した後に`$kei-superset-implementation-handoff`を呼び、表示された事前確認に明示的にOKすること。内部フローは、引継ぎ情報 → `$kei-handoff` → 実装指示 → モデルと推論レベルの選択 → 事前確認 → ユーザー承認 → Superset起動。

このskillが担当するのは、`$kei-handoff`による引継ぎ準備の判定と実装指示への変換、モデル、推論レベル、起動方式の選択、全文の事前確認、明示的な起動承認、起動直前の再確認、安全なコマンド実行、起動証拠、親タスクへの報告だけ。

## 出力言語

- ユーザーへ見せる事前確認、質問、停止理由、完了報告の見出し、説明、表の項目名など、自然言語部分はすべて分かりやすい日本語で書く。
- `$kei-handoff` が作る実装指示も、日本語の自然言語部分をそのまま表示する。
- identifier、path、command、error string、model slug、reasoning valueなど、正確さが必要な技術リテラルは原文を保持する。
- 専門用語が必要な場合は普通の日本語を先に書き、必要な時だけ技術用語を添える。説明のない英語見出しは使わない。
...
- `$kei-handoff`が準備未完了を返した場合は、必要な確認質問をユーザーへ返し、Supersetを起動せず停止する。
...
````markdown
Supersetハンドオフの事前確認です。まだworkspace（作業領域）、agent（エージェント）、terminal（ターミナル）は作成していません。

| # | 対象リポジトリ | ブランチ / HEAD | モデル | 推論レベル | 選択理由 | 起動方式 |
|---:|---|---|---|---|---|---|
| 1 | <owner/repo> | <branch / sha> | <model> | <reasoning> | <日本語1文> | <起動方式> |

許可する外部変更: <許可する操作、または「なし」>

モデル利用可否の確認根拠: <Codex executable、version、catalog command。fallback時だけcache metadata>
...
この内容でSupersetへ委譲してよければ`OK`と明示してください。変更したい場合は、実装指示、モデル、推論レベル、または対象範囲の修正内容を指定してください。
````
...
```markdown
承認済みの内容でSupersetの実装担当エージェントを起動し、Codexの実行開始まで確認しました。

| # | 対象リポジトリ | 作業領域 / セッション | ブランチ / 作業ツリー | モデル | 推論レベル | 起動確認の証拠 |
|---:|---|---|---|---|---|---|
| 1 | <owner/repo> | <IDs> | <ブランチとパス> | <確認済みモデル> | <確認済み推論レベル> | <観測した証拠> |
```

起動できない、承認待ち、承認内容が stale、または起動確認できない場合は成功表現を使わない。

```markdown
Supersetハンドオフは完了していません。

| # | 状態 | 確認済みの証拠 | 次に必要な最小対応 |
|---:|---|---|---|
| 1 | <承認待ち / 承認内容が古い / 未起動 / 起動未確認> | <観測した証拠> | <次に必要な承認、確認、または判断> |
```
