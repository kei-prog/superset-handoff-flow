...
Codex review is primarily a review closeout, not a test orchestrator. If the current turn already has explicit verification evidence, prefer reviewing the diff against that evidence before running more commands.

- Treat recently completed focused tests, format checks, type-checks, and manual command outputs in the current transcript as valid evidence unless the diff changed after they ran.
- Do not rerun broad test suites just because they are available. Run additional commands only when a concrete finding needs reproduction, a required verification is missing, or a prior command is stale.
- If additional verification is needed, state the reason and use the narrowest package-relative command shape for the touched code. For package-filtered Vitest in a pnpm monorepo, prefer paths relative to the selected package root, e.g. `pnpm --filter <package> test src/...`, not repo-root paths such as `apps/<package>/src/...`.
- If the user or parent agent supplied an exact focused verification command, reuse that command rather than inventing a broader one.
- Node/runtime warnings that do not affect the reviewed behavior should be reported as noise or environment mismatch, not expanded into unrelated broad test investigation.
...
Before starting a long inline review, check whether another `codex-review` or
`codex review` process is already running for the same worktree. Do not start a
duplicate review against the same patch. If an old review is still running but
its stdout can no longer be recovered after a context transition, stop the stale
process and rerun the helper with an output file.

For reviews that may run long or span context compaction, write the helper output
to a file so the result is recoverable:

```bash
CODEX_REVIEW_OUTPUT=/tmp/codex-review-<slug>.log ~/.codex/skills/codex-review/scripts/codex-review
```
