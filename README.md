...
Codex の親スレッドで問題・期待結果・制約を明確にし、自己完結した Agent Prompt へ変換して、[Superset](https://superset.sh) 経由で子の Codex agent に repo-scoped な調査・実装を委譲し、レビュー・merge まで安全に回すための skill セットです。
...
通常ユーザーが呼ぶのは `$kei-superset-implementation-handoff` だけです。問題・期待結果・成功条件・制約が会話で明確になり、Blocking unknown が 0 件なら、内部で `$kei-handoff` が Agent Prompt を作ります。不足があれば起動せず、必要な確認を 1 つ返します。
...
| 2 | `kei-superset-implementation-handoff` | 1 を内部利用して Superset を起動し、Codex の実行開始を証拠で確認する |
...
5. 試用:
   ...
   - $kei-superset-implementation-handoff を使って Superset 経由で Codex agent に渡す。内部では $kei-handoff が Ready 判定と Agent Prompt 変換を行い、子 agent が repo を調査して実装する。Codex 上では skill が $skill名 の形で候補表示・発動されることがある。
